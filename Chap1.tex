...
\paragraph{Definition:} Sind V und W K-VR, so heißt eine Abbildung $f: V \rightarrow W$ (K-)Linear oder ein (Vektorraum-)Homomorphismus $f\in Hom(V,W)$, falls gilt:
...
\paragraph{Bemerkung:} Damit die Verträglichkeit mit der Skalarmultiplikation sinnvoll ist, müssen V und W VR über demselben Körper K sein.
  ...
  \paragraph{Beispiel:) Sei V ein K-VR und $y\in K$ fest, dann ist die Streckung um y $y_y:V\rightarrow V: v\mapsto y_y(v) := y$ ein Homomorphismus von V in sich, $y_y\in Hom(V,V)$. Eine Streckung nennt man auch Homothetie.
  	
  	\paragraph{Beispiel:} Sei $V = \mathbb{C} = \{z = x+iy\mid x,y\in \mathbb{R}}$ dann ist die komplexe Konjugation $\mathbb{C}\ni z = x+iy \mapsto x-iy =: \bar{z} \in \mathbb{C}$ kein Homomorphismus von $\mathbb{C}$ in sich, wenn man $\mathbb{C}$ als $\mathbb{C}$-VR auffasst. Hingegen ist sie ein Homomorphismus von $\mathbb{C}$ in sich, wenn man $\mathbb{C}$ als R-VR auffasst.
